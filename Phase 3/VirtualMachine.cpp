...
void VirtualMachine::LOAD(string infile){
	...
	cout << "Test1" << endl;
	string trash;
	//needs to check if this is or is not the first frame being taken by this process.
	if (pageFaults > 0){
		cout << pageFaults << "(vm)" << endl;
		for (int k = 1; k <= (pageFaults*8); k++){	
